...
public class CommonApi implements ModpackApi {
    ...
    private final ModpackApi mTechnicApi;
    private final ModpackApi mATLauncherApi;
    private final ModpackApi mFtbLegacyApi;
    ...
    public SearchResult searchMod(SearchFilters searchFilters, SearchResult previousPageResult) {
        ...
        for(int i = 0; i < mModpackApis.length; i++) {
            ...
            if (api == mTechnicApi || api == mATLauncherApi || api == mFtbLegacyApi) {
                boolean isSearchingForModpacks = "modpack".equals(searchFilters.projectType) || (searchFilters.projectType == null && searchFilters.isModpack);
                if (!isSearchingForModpacks) {
                    continue;
                }
            }
            ...
            futures[i] = OnyxApplication.sExecutorService.submit(new ApiDownloadTask(i, searchFilters,
                    results[i]));
        }
    ...
    private @NonNull ModpackApi getModpackApi(int apiSource) {
        switch (apiSource) {
            ...
            case Constants.SOURCE_TECHNIC:
                return mTechnicApi;
            case Constants.SOURCE_ATLAUNCHER:
                return mATLauncherApi;
            case Constants.SOURCE_FTB_LEGACY:
                return mFtbLegacyApi;
            default:
