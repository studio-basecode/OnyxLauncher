...
public class GLInfoUtils {
    ...
    private static boolean initAndQueryInfo() {
        ...
        // Try GLES3 first with proper EGL_OPENGL_ES3_BIT_KHR config so gl4es backend
        // can detect GLES3 when it's loaded after this context is released.
        int[] egl_attributes_es3 = new int[] {
                EGL14.EGL_BLUE_SIZE, 8,
                EGL14.EGL_GREEN_SIZE, 8,
                EGL14.EGL_RED_SIZE, 8,
                EGL14.EGL_ALPHA_SIZE, 8,
        ...
        EGLConfig[] config3 = new EGLConfig[1];
        int[] num_configs3 = new int[]{0};
        boolean hasEs3Config = EGL14.eglChooseConfig(eglDisplay, egl_attributes_es3, 0, config3, 0, 1, num_configs3, 0)
                && num_configs3[0] > 0;

        int contextGLVersion = 3;
        EGLContext context = null;
        if (hasEs3Config) {
            context = tryMakeCurrent(eglDisplay, config3[0], surface, 3);
        }
        if (context == null) {
            // Fallback: try GLES3 context with ES2 config
            contextGLVersion = 3;
