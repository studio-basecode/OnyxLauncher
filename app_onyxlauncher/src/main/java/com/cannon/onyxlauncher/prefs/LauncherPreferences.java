package com.cannon.onyxlauncher.prefs;
...
public class LauncherPreferences {
    ...
    public static String PREF_APP_LANGUAGE = "default";
    ...
    public static void loadPreferences(Context ctx) {
        ...
        SharedPreferences onyxPrefs = ctx.getSharedPreferences("OnyxData", Context.MODE_PRIVATE);
        PREF_APP_LANGUAGE = onyxPrefs.getString("app_language", DEFAULT_PREF.getString("app_language", "default"));
        ...
        PREF_RAM_ALLOCATION = onyxPrefs.getInt("allocated_ram", DEFAULT_PREF.getInt("allocation", findBestRAMAllocation(ctx)));
    ...
    public static void applyProfileOverrides(com.cannon.onyxlauncher.value.launcherprofiles.MinecraftProfile profile) {
        if (profile == null) return;

        if (profile.ramAllocation != null && profile.ramAllocation > 0) {
            PREF_RAM_ALLOCATION = profile.ramAllocation;
        }
        if (profile.resolutionScale != null && profile.resolutionScale > 0) {
            PREF_SCALE_FACTOR = profile.resolutionScale / 100f;
        }
        if (profile.alternateSurface != null) {
            PREF_USE_ALTERNATE_SURFACE = profile.alternateSurface;
        }
        if (profile.forceVsync != null) {
            PREF_FORCE_VSYNC = profile.forceVsync;
        }
        if (profile.pojavRendererName != null && !profile.pojavRendererName.isEmpty()) {
            PREF_RENDERER = profile.pojavRendererName;
        }
    }
