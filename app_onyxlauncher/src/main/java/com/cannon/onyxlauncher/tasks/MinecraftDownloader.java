package com.cannon.onyxlauncher.tasks;
...
import com.cannon.onyxlauncher.JAssetInfo;
import com.cannon.onyxlauncher.JAssets;
import com.cannon.onyxlauncher.JMinecraftVersionList;
import com.cannon.onyxlauncher.NewJREUtil;
import com.cannon.onyxlauncher.R;
import com.cannon.onyxlauncher.Tools;
import com.cannon.onyxlauncher.mirrors.DownloadMirror;
import com.cannon.onyxlauncher.mirrors.MirrorTamperedException;
...
import com.cannon.onyxlauncher.value.MinecraftClientInfo;
...
public class MinecraftDownloader {
    ...
    private static final int MIN_DOWNLOAD_THREADS = 4;
    private static final int MAX_DOWNLOAD_THREADS = 12;
    ...
    private void downloadGame(Activity activity, JMinecraftVersionList.Version verInfo, String versionName) throws Exception {
        ...
        int downloadThreads = getDownloadThreadCount();
        ...
        ThreadPoolExecutor downloaderPool =
                new ThreadPoolExecutor(downloadThreads, downloadThreads, 500, TimeUnit.MILLISECONDS, taskQueue);
    ...
    private static int getDownloadThreadCount() {
        int processors = java.lang.Runtime.getRuntime().availableProcessors();
        return Math.max(MIN_DOWNLOAD_THREADS, Math.min(MAX_DOWNLOAD_THREADS, processors + 2));
    }
     ...
     */
    private void downloadAndProcessMetadata(Activity activity, JMinecraftVersionList.Version verInfo, String versionName) throws IOException, MirrorTamperedException {
        File versionJsonFile;
        if(verInfo != null) {
            versionJsonFile = downloadGameJson(verInfo);
        } else {
            versionJsonFile = createGameJsonPath(versionName);
            // If the local JSON doesn't exist and verInfo is null (version not in release table,
            // e.g. an inherited vanilla version like 1.21.1 for Fabric), try to fetch it from
            // the Mojang version manifest so it can be downloaded automatically.
            if(!versionJsonFile.canRead() || !versionJsonFile.isFile()) {
                Log.i("NewMCDownloader", "Version " + versionName + " not in release table, fetching from manifest");
                verInfo = fetchVersionInfoFromManifest(versionName);
                if(verInfo != null) {
                    versionJsonFile = downloadGameJson(verInfo);
                }
            }
        ...
        if(activity != null){
            NewJREUtil.installNewJreIfNeeded(activity, verInfo);
        }
        ...
        if(Tools.isValidString(verInfo.inheritsFrom)) {
            ...
            downloadAndProcessMetadata(activity, inheritedVersion, verInfo.inheritsFrom);
        }
    }

    /**
     * Fetch version metadata from the Mojang version manifest for a version that is not
     * present in the locally cached release table. This handles the case where a modloader
     * (e.g. Fabric) references a vanilla version via "inheritsFrom" but that version hasn't
     * been listed yet in memory (e.g. after restarting the app).
     *
     * @param versionId The vanilla Minecraft version ID to look up (e.g. "1.21.1")
     * @return The Version metadata with url and sha1 filled in, or null if not found
     */
    private JMinecraftVersionList.Version fetchVersionInfoFromManifest(String versionId) {
        try {
            String manifestUrl = com.cannon.onyxlauncher.prefs.LauncherPreferences.PREF_VERSION_REPOS;
            Log.i("NewMCDownloader", "Fetching manifest to resolve " + versionId + " from " + manifestUrl);
            ...
            if(versionList != null && versionList.versions != null) {
                ...
                for(JMinecraftVersionList.Version v : versionList.versions) {
                    if(versionId.equals(v.id)) {
                        Log.i("NewMCDownloader", "Found exact " + versionId + " in manifest: " + v.url);
                        return v;
                    }
                }
                // If versionId is a modloader version (e.g. fabric-loader-0.16.10-1.21.1), extract pure version
                java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("(?<![\\d.])1\\.\\d+(\\.\\d+)?(?![\\d.])");
                java.util.regex.Matcher matcher = pattern.matcher(versionId);
                ...
                if(pureVersion != null) {
                    for(JMinecraftVersionList.Version v : versionList.versions) {
                        if(pureVersion.equals(v.id)) {
                            Log.i("NewMCDownloader", "Found inherited base version " + pureVersion + " in manifest for " + versionId);
                            return v;
                        }
                    }
                }
            }
            Log.w("NewMCDownloader", "Version " + versionId + " not found in manifest");
        } catch(IOException e) {
            Log.e("NewMCDownloader", "Failed to fetch manifest for version " + versionId, e);
        }
        return null;
    }
