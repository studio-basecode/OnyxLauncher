...
JNIEXPORT void JNICALL Java_com_cannon_onyxlauncher_utils_JREUtils_setupBridgeWindow(JNIEnv* env, ABI_COMPAT jclass clazz, jobject surface) {
...
Java_com_cannon_onyxlauncher_utils_JREUtils_releaseBridgeWindow(ABI_COMPAT JNIEnv *env, ABI_COMPAT jclass clazz) {
...
void* load_turnip_vulkan() {
    if(getenv("ONYX_LOAD_TURNIP") == NULL) return NULL;
    const char* native_dir = getenv("ONYX_NATIVEDIR");
    const char* cache_dir = getenv("TMPDIR");
    if(native_dir == NULL || cache_dir == NULL) {
        printf("AdrenoSupp: Missing native_dir or cache_dir\n");
        return NULL;
    }
    if(!linker_ns_load(native_dir)) {
        printf("AdrenoSupp: Failed to create driver namespace\n");
        return NULL;
    }
    void* linkerhook = linker_ns_dlopen("liblinkerhook.so", RTLD_LOCAL | RTLD_NOW);
    if(linkerhook == NULL) {
        printf("AdrenoSupp: Failed to load linkerhook\n%s\n", dlerror());
        return NULL;
    }
    ...
    if(dl_android == NULL) {
        printf("AdrenoSupp: Failed to load libdl_android\n%s\n", dlerror());
        dlclose(linkerhook);
    ...
    if(linkerhook_pass_handles == NULL || android_get_exported_namespace == NULL) {
        printf("AdrenoSupp: Failed to resolve linkerhook symbols\n");
        dlclose(dl_android);
        dlclose(linkerhook);
        dlclose(turnip_driver_handle);
        return NULL;
    ...
    if(libvulkan == NULL) {
        printf("AdrenoSupp: Failed to load patched libvulkan\n%s\n", dlerror());
    }
    return libvulkan;
}
#endif
...
int pojavInitOpenGL() {
    ...
    if (pojav_environ->config_renderer == RENDERER_GL4ES) {
        set_gl_bridge_tbl();
    } else if (pojav_environ->config_renderer == RENDERER_VK_ZINK) {
        load_vulkan();
