...
static void replace_in_place(char *str, const char *old, const char *new_w) {
    size_t old_len = strlen(old);
    size_t new_len = strlen(new_w);
    char *pos = str;
    while ((pos = strstr(pos, old)) != NULL) {
        memcpy(pos, new_w, new_len);
        if (new_len < old_len) {
...
static void censor_log_line(char *str) {
    if (!str) return;
    replace_in_place(str, "POJAVEXEC", "ONYXEXEC");
    replace_in_place(str, "pojavexec", "onyxexec");
    replace_in_place(str, "POJAV", "ONYX");
    replace_in_place(str, "Pojav", "Onyx");
    replace_in_place(str, "pojav", "onyx");
}
...
static bool recordBuffer(char* buf, ssize_t len) {
    ...
    censor_log_line(buf);
    ssize_t new_len = strlen(buf);
    if(latestlog_fd != -1) {
        write(latestlog_fd, buf, new_len);
...
Java_com_cannon_onyxlauncher_Logger_begin(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if(latestlog_fd != -1) {
    ...
    if(logger_onEventLogged == NULL) {
        jclass eventLogListener = (*env)->FindClass(env, "com/cannon/onyxlauncher/Logger$eventLogListener");
...
JNIEXPORT void JNICALL Java_com_cannon_onyxlauncher_Logger_appendToLog(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
    ...
    if(recordBuffer(newChars, appendStringLength+1) && logListener != NULL) {
        size_t new_len = strlen(newChars);
        if (new_len > 0 && newChars[new_len - 1] == '\n') {
            newChars[new_len - 1] = 0;
        }
        jstring censoredText = (*env)->NewStringUTF(env, newChars);
        (*env)->CallVoidMethod(env, logListener, logger_onEventLogged, censoredText);
        (*env)->DeleteLocalRef(env, censoredText);
    }
}

JNIEXPORT void JNICALL
Java_com_cannon_onyxlauncher_Logger_setLogListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_cannon_onyxlauncher_utils_JREUtils_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/cannon/onyxlauncher/ExitActivity"));
