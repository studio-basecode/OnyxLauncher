...
bool linker_ns_load(const char* lib_search_path) {
    ...
    android_link_namespaces(driver_namespace, NULL, "ld-android.so:libc.so:libm.so:libdl.so:liblog.so:libnativewindow.so:libsync.so:libhardware.so:libcutils.so:libz.so");
