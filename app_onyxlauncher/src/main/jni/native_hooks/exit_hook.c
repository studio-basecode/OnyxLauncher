...
Java_com_cannon_onyxlauncher_utils_JREUtils_initializeHooks(JNIEnv *env, jclass clazz) {
    ...
    LOGI("initializeHooks: renderer=%s, pojav_environ=%p", renderer ? renderer : "NULL", pojav_environ);
    if (renderer && pojav_environ) {
        if (strncmp("opengles", renderer, 8) == 0) {
            pojav_environ->config_renderer = RENDERER_GL4ES;
        } else if (strcmp(renderer, "vulkan_zink") == 0) {
            pojav_environ->config_renderer = RENDERER_VK_ZINK;
        }
        LOGI("initializeHooks: set config_renderer=%d", pojav_environ->config_renderer);
    }
